...
def kl_annealing(current_epoch, max_epoch=100, max_beta=Config().variational_beta):
    ratio = current_epoch / max_epoch
    if ratio > 1:
        ratio = 1
    return (current_epoch / max_epoch) * max_beta
...
def train_epoch(epoch, model, criterion, optimizer, lr_scheduler, train_dataloader, clip):
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        vq_loss, scale_deformation_recon, perplexity = model(scale_deformation.to(device), one_hot_pose_vector.to(device), img.to(device), img_crop.to(device), img_zoom.to(device))
        ...
        wandb.log({"Train Loss": math.log10(loss_val), "Perplexity": perplexity.item()})
